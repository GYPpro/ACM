...
void solve()
{
    ...
    cin >> n >> m;
    // vector<int> arr(m + 2);
    lazyTree<int> seg(m+2);
    seg.build(vector<int>(m+3,0));
    sumTree<int> sum(m+2);
    sum.build(vector<int>(m+3,0));
    vector<int> t;
    vector<pii> rg;
    lop(i, 0, n)
    {
    ...
    for(int i = n-1;i >= 0;i --)
    {
        curl = max(rg[i].fi,curl);
        curr = min(rg[i].se,curr);
        seg.update(rg[i].fi,rg[i].se,-1);
        sum.update(rg[i].fi,rg[i].se,-1);
        // cout << curl << " " << curr << "\n";
    // lop(i,1,m+1) cout << seg.getsum(i,i) << " " ;cout << "\n";
        int rsl = INF,rsr = INF;
        if(curl > curr){cout << "NO\n";return;}
        if(curr - curl < t[i]) {cout << "NO\n";return;}
        // cout << curl << " " <<curl+t[i]<< ":" << seg.getsum(curl,curl+t[i]) << "\n";
        // cout << curr << " " << curr-t[i] << ":" <<  seg.getsum(curr-t[i],curr) << "\n";
        // if(curl+t[i] <= rg[i].se) rsl = seg.getsum(curl,curl+t[i]);
        if(curl+t[i]-1 <= rg[i].se) rsl = seg.getsum(curl,curl+t[i]-1) * 100000LL + sum.getsum(curl,curl+t[i]-1);
        if(curr-t[i]+1 >= rg[i].fi) rsr = seg.getsum(curr-t[i]+1,curr) * 100000LL + sum.getsum(curr-t[i]+1,curr);
        // if(curr-t[i] >= rg[i].fi) rsr = seg.getsum(curr-t[i],curr);
        // cout << rsl << " " << rsr << "\n";
        if(rsl == INF) 
            if(rsr == INF) {cout << "NO\n";return;}
            else curr = curr-t[i];
        else if(rsr == INF) curl = curl+t[i];
            else {
                if(rsr > rsl) curl = curl+t[i];
                else  curr = curr-t[i];
            }
        // cout << curl << " " << curr << "-\n";
    }
