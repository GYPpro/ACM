...
public:
    void solve()
    {
        ...
        // for(auto &x:itv) cin >> x.fi >> x.se;
        // vector<int> res(32);

        // auto checkALL =  [&](int n) -> bool {
        //     for(auto x:itv) {
        //         if(x.se - x.fi > (1LL << n)) continue;
        //         if(((x.se >> (n)) % 2) || ((x.first >> (n)) % 2)) continue;
        //         // if(x.se >= (1LL << n) && x.fi <= (1LL << n)) continue;
        //         else return 0;
        //         // x.se
        //     }
        ...
        // }
        // int ans = 0;
        // for(int i = 0;i < res.size();i ++) ans += res[i] * (1LL << i);
        // cout << ans << "\n";
    }
};
