...
void solve()
{
    ...
    for(int i=0;i<n;i++){
        ...
        if(l < r)
        {
            cout << l+1 << " " << r-1 << seg.getsum(l+1,r-1) << "\n";
            if(l+1 <= r-1)ans += seg.getsum(l+1,r-1);
        } 
        else {
            int l1 = l,r1 = n;
            int l2 = 1,r2 = r;
            // if(l1+1 <= r1)cout << l1+1 << " " << r1 << seg.getsum(l1+1,r1) << "\n";
            // if(l2 <= r2-1)cout << l2 << " " << r2-1 << seg.getsum(l2,r2-1) << "\n";
            if(l1+1 <= r1)ans += seg.getsum(l1+1,r1);
            if(l2 <= r2-1) ans += seg.getsum(l2,r2-1);
        }
        // for(int j=1;j<k;j++){
        //     ans+=a[(i+j)%n];
        // }
        ans++;
        // a[i]++;
        seg.update(i+1,i+1,1);
        seg.update(((i+k)%(n))+1,r,1);
        // a[(i+k)%n]++;
    }
    cout<<ans<<"\n";
}
