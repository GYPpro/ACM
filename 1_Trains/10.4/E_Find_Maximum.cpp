...
// #define INT_2 vector<vector<int>>
...
void solve()
{
    ...
    //     for (int j = 0; j < i; j++)
    //         pvArr.push_back(2);
    //     if(pvArr.size() < thl.size()) pvArr.push_back(thl[i] - 1);
    //     for (int j = i + 1; j < thl.size(); j++)
    //         pvArr.push_back(thl[i]);

    //     // for(int j = 0;j < pvArr.size();j ++) cout << pvArr[i] << " ";

    //     ll int pv = arrtoInt(pvArr);
    //     if(pv > l || pv < r) continue;
    //     int anst = 0;
    //     for(int j = 0;j < pvArr.size();j ++) anst += pvArr[j] +1;
    //     ans = max(ans,anst);
    //     // cout << i << " " << ans << endl;
    // }
