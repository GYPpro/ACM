...
void solve()
{
    ...
    // void update
    function<int(int)> gnc = [](int n) -> int{
        int t = 0;
        while(n) {t += n%10;n /= 10;}
        return t;
    };
    set<int> usv;
    lop(i,0,n) usv.insert(i);
    while(q--)
    {
        int op;
        cin >> op;
        if(op == 1){
            ...
            cin >> l >> r;
            l --,r--;
            auto iy = usv.lower_bound(l);
            vector<int> ed;
            for(;(*iy) <= r && iy != usv.end();++iy)
            {
                // cout << (*iy) << " ";
                arr[*iy] = gnc(arr[*iy]);
                if(gnc(arr[*iy]) == arr[*iy]) ed.pb(*iy);
            }
            for(auto x:ed) usv.erase(x);
            // cout << "\n";
        } else {
            int t;
            cin >> t;
