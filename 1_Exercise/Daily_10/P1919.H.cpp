...
namespace bitIntNS {
const int32_t COMPRESS_MOD = 1000000;
const uint32_t COMPRESS_DIGITS = 6;

const uint32_t BIGINT_MUL_THRESHOLD = 1;
...
public:
    bitInt() { set(0); }
    explicit bitInt(int n) { set(n); }
    explicit bitInt(intmax_t n) { set(n); }
    explicit bitInt(const char *s) { from_str(s); }
    ...
    BigInt_t operator%(const BigInt_t &b) const { return __MV(*this - *this / b * b); }
